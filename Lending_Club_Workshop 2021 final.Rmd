...
```{r, load_libraries, include = FALSE, message=FALSE, eval=FALSE}
...
```{r, data_visualisation, message=FALSE, eval=FALSE}
...
lc_clean <- lc_clean %>% 
  mutate(date = as.yearmon(issue_d,"%m/%Y"))

lc_clean_time <- lc_clean %>% 
  select(int_rate, date) %>%
  filter(date != "Jun 2007") %>% # filtering (add comment)
  group_by(date) %>% 
  summarise(avg_int_rate = mean(int_rate))

lc_clean_time %>% 
  ggplot(aes(x = date,y = avg_int_rate))+
  geom_line()+
  labs(x = "Month-Year",
