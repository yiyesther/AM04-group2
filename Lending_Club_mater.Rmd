...
ggplot(lc_clean, aes(x=int_rate))+  
  ...
  labs(title = "Histogram of Interest Rates",x="Interest Rate")
```

The first histogram may tell us something about the distribution of the interest rates among the borrowers, but does not provide any additional information beyond that.

```{r,histogram_color,message=FALSE,warning=FALSE}
#histogram with colour for different grades
ggplot(lc_clean, aes(x=int_rate, fill=grade))+  geom_histogram(binwidth=0.01)+scale_x_continuous(labels = scales::percent)+ labs(title = "Histogram of Interest Rates by Loan Grade",x="Interest Rate") 

```

The second histogram however, provides much deeper insight into what the loans provided, clearly showing a relationsip between the grade of a loan and the associated interest rate. 
...
ggplot(lc_clean[seq(1, nrow(lc_clean), 10), ] , aes(x = int_rate, y = loan_amnt)) + 
  geom_jitter(width = 0.003, height = 0.002,
              size = 0.5) +
  geom_smooth(method="lm", se=0) + 
  labs(title = "Interest Rate by Loan Amount", y="Loan Amount ($)", x="Interest Rate")
```

While the line of fit implies a relationship between the loan amount and the interest rate, the distribution of the points in the scatter plot says otherwise. This plot does not provide any meaningful insight.
...
ggplot(lc_clean[seq(1, nrow(lc_clean), 10), ] , aes(x = int_rate, y = annual_inc)) + 
  geom_point(size=0.2, alpha=0.5)+ 
  geom_smooth(method="lm", se=0) +
  ylim(15000, 40000)+
  labs(title = "Interest Rate by Income", y="Annual Income ($)", x="Interest Rate")

```

The nearly flat of this scatter plot and line of fit, also does not provide us with much useful information to work with. A transformation might be needed to better visualise the relationship.
...
We can clearly see from this plot that the number of delinquencies within the past 2 years indeed increases the interest rate charged, up until the 4th delinquency. 
...
ggplot(lc_clean, aes(x=int_rate, fill=grade, alpha = 0.2))+  
  ...
  theme(legend.position = "none")+
  scale_x_continuous(labels = scales::percent)+ 
  labs(title = "Interest rate density across the various loan grades", x="Interest Rate",y="Density")
```
...
ggplot(lc_clean, aes(x=home_ownership, y=int_rate, fill=home_ownership))+  
  ...
  labs(title = "Interest Rates split by Homeownership", y="Interest Rate", x="Home Ownership")+
  NULL
```

The results of the density plot are in line of what we would expect from the lending club. We expect that the different loan grades would actually result in a different interest rate classification, as the density plot clearly shows. In our second visualization, it can be seen that interest rates for those who own a house or have a mortgage are lower, as they have sufficient collateral in the case of default. Renters on average do not possess that collateral, which is reflected in higher interest rates. 
...
# In our simple regression model we analyse the dependence of the interest rate on the amount and length of the loan as well as the annual income and loan grade ranking of the borrower. 
```
...
>
* a. All variables are statistically significant except for the coefficient annual income. 
* b. The loan amount coefficient tells us that for every 100k in loan value, the interest rate rises by 1%. If the loan term is 60 months, the interest rate is increased by another 3.6%.The debt to income ratio will affect the borrowers interest rate at a factor of 0.0432%. The grade the loan receives will add varying degrees of additional interest to the baseline intercept at 7.16%. 
* c. The model explains 91.97 % of the variability in the data set. We can therefore say that the explanatory value is fairly strong. 
* d. The width of the 95% confidence interval will be 2*1.96*Residual Standard Error$ = $2*1.96*0.01056$ = $0.0413952. 
...
Adding the variable interaction Loan Amount x Grade, we were able to add statistically significant coeffeicients and thereby lower the F-Stat, thus increasing the fit of the model.
...
While we have not been able to increase the explanatory power of the model, our adjustments have improved the fit by lowering our F-statistic. Our introduced poly function has failed to make our annual income variable statistically significant. 
...
Breaking the annual income variable into quartiles, we are able to derive new statistically significant coefficients, thus further increasing the fit of our model (lowering the F-Stat).
...
#for some regional date/number (locale) settings this may not work. 
#If it does try running the following line of code in the Console
#Sys.setlocale("LC_TIME","English")
bond_prices <- bond_prices %>%
  mutate(Date2=as.Date(paste("01",Date,sep="-"),"%d-%b-%y")) %>%
  select(-starts_with("X"))

#let's see what happened to bond yields over time. 
#Lower bond yields mean the cost of borrowing has gone down.
bond_prices %>%
...
bestmodel<-train(
    int_rate ~ loan_amnt + term + loan_amnt:grade + log(annual_inc) + grade + 
    Price + grade:Price , #fill your variables here 
    lc_with_bonds,
   method = "lm",
    trControl = control
   )
...
cpi_clean <- cpi %>% 
  ...
  #convert the DATE variable into quarters
  mutate(cpi_delta = 
           (CPALTT01USQ657N - lag(CPALTT01USQ657N))/lag(CPALTT01USQ657N), 
           yq = as.yearqtr(cpi$DATE, format = "%Y-%m-%d")) %>%  
  #filter for the desired date that match with our previous tables
  filter(date >= "2007-07-01" & date <= "2011-12-01") %>% 
  #select the desired columns
  select(date,yq,cpi_delta,CPALTT01USQ657N) %>% 
...
int_cpi <- lc_with_bonds %>% 
  group_by(qtr) %>% 
  #calculate mean quarterly interest rate
  summarise(mean_int_rate = mean(int_rate)) %>% 
  #calculate change in interest rate
  mutate(int_delta = (mean_int_rate - lag(mean_int_rate))/lag(mean_int_rate)) %>% 
...
lc_with_cpi <- lc_with_bonds %>% 
  #arrange by issue date so that we can use fill to autofill values for months with NA
  arrange(issue_d) %>% 
...
int_cpi %>% 
  ...
  #convert x axis to percentage scale
  scale_x_continuous(labels = scales::percent)+ 
