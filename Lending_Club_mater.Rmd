...
temp <- temp %>% 
  mutate(cont_date = as.numeric(difftime(temp$issue_d,min,units='days'))/30)

time1<-train(
  int_rate ~ loan_amnt + term + loan_amnt*grade + log(annual_inc) +
    cont_date, #fill your variables here "+ issue_d"
  temp,
  method = "lm",
  trControl = control)
...
#time2<-train(
    #int_rate ~ loan_amnt + term + loan_amnt*grade + log(annual_inc) + grade + 
    #grade:issue_d + issue_d, #fill your variables here 
    #lc_clean,
   #method = "lm",
    #trControl = control)
...
time2<-train(
    int_rate ~ loan_amnt + term + loan_amnt*grade + log(annual_inc) + 
      grade +grade:cont_date, #fill your variables here 
    temp,
   method = "lm",
    trControl = control
   )
...
time4<-train(
    int_rate ~  loan_amnt + term + loan_amnt*grade + log(annual_inc) + grade +
    yq + grade:yq, #fill your variables here 
    lc_clean_quarter,
     method = "lm",
    trControl = control
   )
...
Although time trend is a significant variable in the previous model, it is not predictive in future trends of interest rates. Instead of using time as a variable, bond price has more explanatory power for the model. 
...
cpi_clean <- cpi %>%
  ...
  select(date,cpi_delta,CPALTT01USQ657N) %>% 
  rename(cpi = CPALTT01USQ657N) #select the desired coloumns
...
lc_with_cpi %>% 
  ggplot(aes(x = cpi_delta, y = int_rate,color=Grade)) +
  geom_point(size = 0.8) +
  ...
  labs(title = "How interest rates vary with change in consumer price index", 
       subtitle = "in different credit grade groups",
...
By including consumer price index, we can include information on inflation thus
